...
| 创建执行清单 | `todo-list-csv` + 读 `AGENTS.md` §List 文档自动执行协议 | "创建list文档", "做个list", "建个执行清单", "list一下" |
...
2. **Read project AGENTS.md** (if exists) for project-specific rules that override this global file.
...
If `AGENTS.md` exists in the project root, read it at session start. It contains project-specific rules that override or extend this global file. Key sections to look for:
- **List 文档自动执行协议** — 触发 "创建list文档" 时的完整执行流程（三组解耦拆分、完成即删等）
- **子规则索引** — 按场景加载对应的 AGENTS.*.md 子文件
...
当项目级 `AGENTS.md` 定义了 "List 文档自动执行协议" 时，遵循其三组解耦拆分规则。
无特殊要求时，所有 item 统一归入组 `A`，按顺序执行。
...
遵循项目级规定。若项目 `AGENTS.md` 或 `AGENTS.trellis.md` 禁止 AI commit，则 AI 只做代码修改，由人工审核后提交。无项目级规定时，AI 可自主 commit。

## 上下文恢复

中断后恢复：扫描 `* TO DO list.csv` → 按 group 分组 → 找到首个 `TODO` / `IN_PROGRESS` → 读对应 `* 规划.md` → 恢复执行。
