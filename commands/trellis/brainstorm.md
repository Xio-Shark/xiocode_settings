...
```bash
TASK_DIR=$(python3 "$TRELLIS_SCRIPTS/"task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
```bash
# Create child tasks
CHILD1=$(python3 "$TRELLIS_SCRIPTS/"task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 "$TRELLIS_SCRIPTS/"task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 "$TRELLIS_SCRIPTS/"task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
